...
import com.neytron.sshcommander.data.CommandHistoryEntity
...
class SshWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        val server = AppDatabase.getDatabase(applicationContext).serverDao().getServerById(serverId) ?: return Result.failure()
        ...
        return try {
            ...
            // Log to history
            AppDatabase.getDatabase(applicationContext).serverDao().insertHistory(
                CommandHistoryEntity(serverId = serverId, command = command, output = result)
            )
    ...
    private fun performSshCommand(server: com.neytron.sshcommander.data.Server, password: String, command: String): String {
        val jsch = JSch()
